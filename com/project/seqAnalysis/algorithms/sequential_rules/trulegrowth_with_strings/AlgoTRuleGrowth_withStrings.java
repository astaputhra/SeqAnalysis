package com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth_with_strings;
...
import com.project.seqAnalysis.input.sequence_database_list_strings.Sequence;
import com.project.seqAnalysis.input.sequence_database_list_strings.SequenceDatabase;
 ...
 *  
 * @see Itemset
 * @see com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.AlgoTRuleGrowth
 *@see Sequence
 *@see com.project.seqAnalysis.input.sequence_database_list_strings.SequenceDatabase
...
public class AlgoTRuleGrowth_withStrings {
	...
	Map<String,  Map<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence>> mapItemCount;
	...
	public void runAlgorithm(String input, String output, int relativeMinSupport, double minConfidence, int windowSize 
		...
		// for each item
		for(Entry<String,Map<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence>> entry : mapItemCount.entrySet()){
			// if it is frequent
		...
		for(int i=0; i< listFrequents.size(); i++){
			...
			Map<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence> occurencesI = mapItemCount.get(intI);
			for(int j=i+1; j< listFrequents.size(); j++){
				String intJ = listFrequents.get(j);
				Map<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence> occurencesJ = mapItemCount.get(intJ);
	...
	looptid:	for(com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence occI : occurencesI.values()){
					// add the sequenceID to tidsI
					tidsI.add(occI.sequenceID);
					
					// if J does not appear in that sequence continue loop
					com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence occJ = occurencesJ.get(occI.sequenceID);
					if(occJ == null){
						continue looptid;
					}
			...
			loopIJ:	for(Short posI : occI.occurences){
				...
				if(tidsIJ.size() >= minsuppRelative){
					...
					for(com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence occJ : occurencesJ.values()){
...
//				Rule ruleI_JC = new Rule(ruleIJ.getItemset1(), itemsetJC, confI_JC, tidsI_JC.size());
	 ...
	 */
	private Map<String, Map<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence>> removeItemsThatAreNotFrequent(SequenceDatabase database) {
		// (1) Count the support of each item in the database in one database pass
		mapItemCount = new HashMap<String, Map<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence>>(); // <item, Map<tid, occurence>>
		...
		for(Sequence sequence : database.getSequences()){
			...
			for(short j=0; j< sequence.getItemsets().size(); j++){
				...
				for(int i=0; i< itemset.size(); i++){
					...
					Map<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence> occurences = mapItemCount.get(itemI);
					if(occurences == null){
						occurences = new HashMap<Integer, com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence>();
						mapItemCount.put(itemI, occurences);
					}
					com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence occurence = occurences.get(sequence.getId());
					if(occurence == null){
						occurence = new com.project.seqAnalysis.algorithms.sequential_rules.trulegrowth.Occurence(sequence.getId());
