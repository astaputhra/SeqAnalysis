package com.project.seqAnalysis.algorithms.frequentpatterns.apriori_HT;
...
//		System.out.println("database size = " +database.size() + "  minsuprel = " + minsupRelative);
		...
		do{
			...
			for(ItemsetHashTree.LeafNode node  = candidatesK.lastInsertedNode;node != null; node = node.nextLeafNode){
	...
	private ItemsetHashTree generateCandidateSizeK(ItemsetHashTree candidatesK_1, int k) {
		...
		// For each leaf node
		for(ItemsetHashTree.LeafNode node  = candidatesK_1.lastInsertedNode; node != null; node = node.nextLeafNode){
